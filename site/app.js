...
function renderDag(nodes) {
  stateElements.dagGraph.innerHTML = "";

  nodes.forEach((node) => {
    const item = document.createElement("li");
    const dependencyText = node.dependsOn.length === 0 ? "Starts the graph" : `Depends on ${node.dependsOn.join(", ")}`;
    item.className = "dag-item";
    item.innerHTML = `
      <strong>${node.name}</strong>
      <span>${dependencyText}</span>
      <small>${node.minutes} min • ${node.automated ? "Automated" : "Manual"}</small>
    `;
    stateElements.dagGraph.append(item);
  });
}
